...
Gemini API (gemini-3.0-flash):

| 項目 | 料金 |
|:---|:---|
| 入力トークン | $0.50/100万トークン |
| 出力トークン | $3.00/100万トークン |
| 入力トークン（画像） | $0.50/100万トークン |
| 無料枠 | あり（AI Studioで確認可能） |

1リクエストあたりの想定トークン数:
- 画像分析: 入力 約2,100トークン（画像+プロンプト）、出力 約500トークン
- テキスト検索: 入力 約200トークン、出力 約300トークン
...
シナリオ1: 通常利用（1日3〜4回の記録）

| 項目 | 月間使用量 | 無料枠 | コスト |
|:---|:---|:---|:---|
| Cloud Run | 3,500回 | 200万回 | $0 |
| Firestore読み取り | 35,000回 | 150万回/月 | $0 |
| Firestore書き込み | 7,000回 | 60万回/月 | $0 |
| Cloud Storage | 300MB | 5GB | $0 |
| Gemini API | 252Kトークン（120回×2.1K入力） | 無料枠内 | $0 |

月額: $0（無料枠超過時: 約$0.30）

シナリオ2: ヘビー利用（1日6回以上の記録）

| 項目 | 月間使用量 | 無料枠 | コスト |
|:---|:---|:---|:---|
| Cloud Run | 6,000回 | 200万回 | $0 |
| Firestore読み取り | 60,000回 | 150万回/月 | $0 |
| Firestore書き込み | 12,000回 | 60万回/月 | $0 |
| Cloud Storage | 500MB | 5GB | $0 |
| Gemini API | 378Kトークン（180回×2.1K入力） | 無料枠内 | $0 |

月額: $0（無料枠超過時: 約$0.46）

#### 無料枠を超える可能性があるケース

Gemini APIのコスト試算（無料枠超過時）:
- 入力: 0.189Mトークン × $0.50 = $0.09
- 出力: 0.045Mトークン × $3.00 = $0.14
- 合計: 約$0.23/月（通常利用シナリオ）
...
- バックエンドAPIのE2Eテスト（backend/e2e/）が統合テストを担う
