...
ウチコミは現在、食事画像やテキストから栄養素を自動推定し記録する機能を持つ。しかし「この後、何を食べればいいのか」を提案する機能がなく、ユーザーは自分で食事内容を決める必要がある。

選手がパフォーマンスを維持し、体重を調整・減量していく過程では、適切な食材を自分で調理して食べることが重要である。プロテインや加工済みサプリメントではなく、食事で栄養を整えるべきだが、個人でそのような管理を行うのは難しい。自炊メニュー機能は、冷蔵庫の食材と身体データに基づいて「何をどう食べるべきか」をサジェストし、この課題を解決する。

### 背景にある課題

- 体重調整・減量中に「次に何を食べればいいか分からない」
- 栄養目標に沿った献立を自分で考える負担が大きい
- 自炊で栄養管理を行いたいが、食材の在庫と栄養バランスの両方を考慮した献立作成が困難
...
初期実装では単発のGemini API呼び出しでコンテキストを渡し、回答を得る形とする。

将来的には、体重・食事記録・栄養目標などの登録情報をベースに継続的なコンテキストを保持し、より精度の高い提案を行うAIエージェント的なモジュールの導入を想定している。その場合、メニュー提案ロジックを独立したモジュール（例: `internal/agent/` や `pkg/agent/`）に切り出し、各種データソースからのコンテキスト収集とGemini API呼び出しを統合する構成になる見込みである。

初期実装のService層（`pkg/gemini/menu_suggester.go`）を将来のエージェントモジュールへ段階的に移行できるよう、インターフェースで抽象化しておく。
