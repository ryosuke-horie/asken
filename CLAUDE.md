...
- 後戻りしにくい変更を実行する前に、必ずAskUserQuestionツールでユーザーの許可を得ること
  - 例: Linearイシューのクローズ/ステータス変更、ラベルの一括変更、ファイルの削除、データベーススキーマ変更、インフラ変更（terraform apply）等
  - 調査・分析・読み取りのみの操作は許可不要
