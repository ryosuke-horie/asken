...
- 環境依存を理由にエラーを無視して「完了」と報告すること
  - 環境依存の問題でも根本原因を特定し、修正または対処方法を明示すること
  - 「環境の問題なので諦めます」は許可されない
- ビルドが実際に通っていないのに「できています」と報告すること
  - ビルドコマンドの出力を必ず確認し、成功を実証すること
- エラーを隠蔽するワークアラウンド（テストのスキップ、警告の抑制等）
  - 根本原因を修正すること
...
1. フレーキーの原因を特定し、ユーザーに報告すること
2. 再現条件を明記すること（タイミング依存、キャッシュ依存等）
3. 根本的な修正が可能なら修正する。不可能なら原因と対処方法を説明する

### ユーザーへのエスカレーション

以下の場合はユーザーに理由を添えて依頼すること:

- macOSの再起動が必要な場合（Xcodeプロセスのハング、シミュレータの不整合等）
- Xcodeの手動操作が必要な場合（Signing設定、手動のPackage Resolve等）
- 環境変数やシステム設定の変更が必要な場合
