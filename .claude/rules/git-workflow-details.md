...
| ラベル | 用途 | 判断基準 |
|:---|:---|:---|
| `Feature` | 新機能・大きめの機能改善 | iOSを触りながらローカルでビルド・手動チェックが必要なタスク |
| `iOS` | iOS修正を含むタスク | iOSアプリの修正・実装が確実に入るタスク |
| `Terraform` | インフラ変更を含むタスク | Terraformによるapply・認証が必要なタスク |
| `Bug` | バグ修正 | 既存機能の不具合修正 |
| `documentation` | ドキュメント関連 | ドキュメントの作成・更新 |
| その他 | 必要に応じて追加 | - |

## iOS開発タスクの管理

- iOSを複数並列で動かすことは避け、一つの専用ローカルリポジトリで対応する
- iOSの修正が確実に入ると思われるタスクには、一目でわかるように「iOS」ラベルを貼る
- ラベルがないタスクについては、CI上やローカルのコード実行のみで動作確認が完結するものとし、そのまま進めても構わない

## Terraformタスクの管理

- Terraformの修正（apply）が必要なタスクには「Terraform」ラベルを貼る
- 認証が必要なため、メインで取り組みたいタスクとして分類する
- infra/ ディレクトリ以下の変更を伴うタスクが該当
...
PRを作成してpushした後は、CIの結果を監視し、失敗した場合は即座に対応すること。
...
# 最新のCI runを確認
gh run list --branch edg-xxx
```

### 注意点

- CIが成功するまで次の作業に進まないこと
- 失敗のまま放置して他の作業を開始しないこと
- CIが失敗しているPRはマージしないこと
