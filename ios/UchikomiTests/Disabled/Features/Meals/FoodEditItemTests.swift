...
@Suite struct FoodEditItemTests {
    ...
    @Test func 単位が変更された場合hasUnitChangedがtrueになるべき() {
        let item = FoodEditItem(
            name: "白米",
            quantity: "100g",
            calories: 168,
        ...
        // NutritionInfoから初期化してquantityValueとquantityUnitを設定
        let info = NutritionInfo(
            name: "白米",
            estimatedAmount: "100g",
            caloriesKcal: 168,
        ...
        let itemFromInfo = FoodEditItem(from: info)

        // 単位を変更
        itemFromInfo.quantityValue = "1"
        itemFromInfo.quantityUnit = .cup

        #expect(itemFromInfo.hasUnitChanged == true)
    }

    @Test func 杯から合への変更でhasUnitChangedがtrueになるべき() {
        let info = NutritionInfo(
            name: "白米",
            estimatedAmount: "1杯",
            caloriesKcal: 168,
        ...
        item.quantityUnit = .go

        #expect(item.hasUnitChanged == true)
    }

    @Test func 数値のみ変更の場合hasUnitChangedがfalseになるべき() {
        let info = NutritionInfo(
            name: "白米",
            estimatedAmount: "100g",
            caloriesKcal: 168,
        ...
        // 数値のみ変更
        item.quantityValue = "200"
        item.quantityUnit = .gram

        #expect(item.hasUnitChanged == false)
    }

    @Test func quantityUnitがnilの場合hasUnitChangedがfalseになるべき() {
        let item = FoodEditItem(
            name: "白米",
            quantity: "100g",
            calories: 168,
    ...
    // MARK: - updateQuantityString テスト

    @Test func updateQuantityStringが正しくquantityを生成するべき() {
        let item = FoodEditItem()
        item.quantityValue = "100"
        item.quantityUnit = .gram
    ...
    @Test func updateQuantityStringで合単位が正しく生成されるべき() {
        let item = FoodEditItem()
        item.quantityValue = "1"
        item.quantityUnit = .go
        item.updateQuantityString()

        #expect(item.quantity == "1合")
    }

    @Test func quantityUnitがnilの場合quantityは空になるべき() {
        let item = FoodEditItem()
        item.quantityValue = "100"
    ...
    @Test func quantityValueが空の場合quantityは空になるべき() {
        let item = FoodEditItem()
        item.quantityValue = ""
        item.quantityUnit = .gram
        item.updateQuantityString()
    ...
    // MARK: - NutritionInfoからの初期化テスト（quantityValueとquantityUnit）

    @Test func NutritionInfoからの初期化でquantityValueとquantityUnitが設定されるべき() {
        let info = NutritionInfo(
            name: "白米",
            estimatedAmount: "100g",
            caloriesKcal: 168,
        ...
        #expect(item.quantityValue == "100")
        #expect(item.quantityUnit == .gram)
    }

    @Test func NutritionInfoからの初期化で合単位が正しくパースされるべき() {
        let info = NutritionInfo(
            name: "白米",
            estimatedAmount: "2合",
            caloriesKcal: 336,
        ...
        #expect(item.quantityValue == "2")
        #expect(item.quantityUnit == .go)
    }

    @Test func NutritionInfoからの初期化で小数値が正しくパースされるべき() {
        let info = NutritionInfo(
            name: "鶏むね肉",
            estimatedAmount: "150.5g",
            caloriesKcal: 248,
            proteinG: 46,
            fatG: 5.4,
            carbohydratesG: 0
        ...
        #expect(item.quantityValue == "150.5")
        #expect(item.quantityUnit == .gram)
    }
}
